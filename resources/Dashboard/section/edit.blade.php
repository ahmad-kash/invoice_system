<x-layouts.app>
    <x-slot:title>تعديل القسم</x-slot:title>
    <x-slot:breadcrumb>
    ...
    <div class="card card-primary mx-3 my-2">
        <!-- /.card-header -->
        <!-- form start -->
        <form class="p-3" method="POST" action="{{ route('sections.update', $section->id) }}">
            @csrf
            @method('put')
            <div class="card-body">
                <div class="form-group">
                    ...
                    <x-text-input id="name" type="name" name="name"
                        value="{{ old('name') ?? $section->name }}" required autocomplete="name" autofocus
                ...
                <div class="form-group">
                    ...
                    <x-textarea id="description" name="description" autocomplete="description" rows="5"
                        :hasError="$errors->has('description')">{{ old('description') ?? $section->description }}</x-textarea>
